...
const lastfm_api = require('../external-services/lastfm.service')
...
const getTopTagsFromDb = (callback) => {
    ...
    GenreModel.Tags.find({},[],{skip: 0, limit: 50, sort: { popularity: -1, songCounter: -1 }}, (err, tagList) => {
        if (err) {
            callback (err);
        } else {
...
const saveLastFmTagToDbIfNeeded = (tag) => {
    checkIfTagExist(tag.name, (tagsCount) => {
        ...
        if(tagsCount === 0) {
            var newTag = new GenreModel.Tag(tag.name, tag.count);
            newTag.addLastFmObject(tag);
            var genremodel = new GenreModel.Tags(newTag);
            genremodel.save();
...
const checkIfTagExist = (tagName, callback) => {
    GenreModel.Tags.count({"name": new RegExp('\\b^' + tagName + '$\\b', 'i')}, (err, tagsCount) => {
        if (err) {
            callback(err);
        } else {
            callback(tagsCount);
